...
import { formatSplits } from '@/utils'

const props = defineProps(['sender', 'newRecipient'])
...
const splits = ref([])
const splitsFormatted = computed(() => formatSplits(splits.value))

const addSplit = () => splits.value.push({ address: '', percent: null })
...
const getSplits = async () => {
  try {
    ...
    // resolve any ENS names...
    for (var i = 0; i < splits.length; i++) {
      splits[i].ens = await store.dispatch('resolveAddress', { address: splits[i].address })
    }
...
const update = async () => {
  try {
    ...
    // submit...
    console.log('currrec', currentReceivers)
    debugger
    tx.value = await store.dispatch('updateAddressSplits', {
      currentReceivers,
      newReceivers: splitsFormatted.value
    })
    console.log('update splits tx', tx)
    ...
    console.error(e)
    alert('Error updating Drip Shares:', e.message || e)
  }
}
  ...
  try {
    ...
    if (props.newRecipient) {
      ...
      const existsAtIndex = splits.value.findIndex(split => {
        const newRecipient = props.newRecipient.address.toLowerCase()
        return split.address === newRecipient || split.ens === newRecipient
      })
      ...
      form(@submit.prevent="update", validate)
        ...
        template(v-for="(split, i) in splits")
          section.my-10.input-group.relative
            input-body(label="% of Revenue", :isFilled="typeof splits[i].percent === 'number'", theme="dark")
              input(v-model="splits[i].percent", type="number", min="0.01", max="100", step="0.01", placeholder="5", required)
            //- input address
            .mt-10
              input-body(label="Ethereum/ENS Address", :isFilled="splits[i].address === 'length'", theme="dark", format="code")
                //- TODO: validate ethereum address
                input(v-model="splits[i].address", placeholder="name.eth", autocomplete="new-password", required)
            //- rmv btn
            .absolute.top-0.right-0.h-full.flex.items-center
              button.w-32.h-32.flex.items-center.justify-center.-mr-12.bg-indigo-900.rounded-full.border-violet-700.border-2.notouch_hover_border-white(@click="removeSplit(i)")
